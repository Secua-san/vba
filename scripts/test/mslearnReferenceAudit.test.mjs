...
import { dialogSheetCommonCallableMemberNames } from "../lib/supplementalReferenceConfig.mjs";
...
test("DialogSheet 補助ソースは allow list の common callable だけを保持する", async () => {
  const data = await loadReferenceData();
  const { owner, memberNames } = getOwnerMemberNames(data, "DialogSheet");
  const normalizedAllowedMemberNames = new Set(
    [...dialogSheetCommonCallableMemberNames].map((memberName) => normalizeMemberName(memberName)),
  );
