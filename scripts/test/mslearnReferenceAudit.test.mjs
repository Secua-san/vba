...
test("WorksheetFunction の監視対象メンバーは追加時に再生成手順書へ誘導する", async () => {
  const data = await loadReferenceData();
  ...
  assert.equal(
    memberNames.has("xlookup"),
    false,
    "WorksheetFunction.XLookup が Learn スナップショットへ追加されました。docs/process/mslearn-signature-regeneration.md に従って allow list / 再生成 / server・extension テストを更新してください。",
  );
  assert.equal(
    memberNames.has("xmatch"),
    false,
    "WorksheetFunction.XMATCH が Learn スナップショットへ追加されました。docs/process/mslearn-signature-regeneration.md に従って allow list / 再生成 / server・extension テストを更新してください。",
  );
});
