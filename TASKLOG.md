...
`TASKS.md` から分離した参照用ログ。詳細な完了履歴や長い補足はここに残し、普段の参照は `TASKS.md` を使う。

## 参照ルール

- 直近の状況、次に行うタスク、重要な運用メモは `TASKS.md` を見る。
- 過去の完了履歴、docs-only の判断記録、長い補足を辿るときだけ `TASKLOG.md` を開く。
...
- [x] Phase 6 の Diagnostics を完了扱いにする
  ...
  - `packages/core/src/diagnostics/byRefDiagnostics.ts` で statement 全体の invocation scan fallback を `executableStatement` に限定して明示した
  - `packages/core/test/analysis.test.ts` で labeled structured statement と unreachable reset の回帰を固定した
  - `npm run build --workspace @vba/core`、`npm run test --workspace @vba/core` が成功した
...
- [x] `TASKS.md` と `TASKLOG.md` の役割分離を導入する
  ...
  - [AGENTS.md](AGENTS.md)、[README.md](README.md)、[docs/README.md](docs/README.md)、[docs/adr/README.md](docs/adr/README.md)、[docs/process/README.md](docs/process/README.md)、[docs/process/mslearn-signature-regeneration.md](docs/process/mslearn-signature-regeneration.md) に役割分担ルールを反映した
  - docs-only の運用更新依頼として実施し、build / lint / test ではなく相互参照と役割分担の整合を確認した


- [x] extension host で active workbook identity snapshot と shared semantic test の安定化を完了する
  - `packages/extension/package.json` の `build` / `test` を見直し、workspace 単体実行でも server bundle と reference/resource copy を含む runnable な test/package 導線へ寄せた
  - `packages/extension/src/extension.ts` と `packages/extension/test/suite/index.ts` で active workbook identity test command の ready/retry/restore を詰め、server state object を invalid payload として再送しないようにした
  - `packages/server/src/lsp/documentService.ts` では semantic token の同一 range 重複を 1 件へ集約し、function return の synthetic declaration token を除外して overlap を防いだ
  - `packages/extension/test/suite/index.ts` の negative hover / signature wait を短縮し、extension host suite の完走時間を実用水準まで戻した
  - `npm run test:scripts`、`npm run test --workspace @vba/core`、`npm run test --workspace @vba/server`、`npm run lint`、`npm run package`、`npm run test --workspace vba-extension` が成功した

- [x] ProcedureStatementNode の assignment / call structured AST first slice を導入する
  - `packages/core/src/types/model.ts` で `ProcedureStatementNode` を union 化し、`assignmentStatement` / `callStatement` と最小 sub-range (`targetRange` / `expressionRange` / `nameRange` / `arguments`) を追加した
  - `packages/core/src/parser/parseModule.ts` で single-line の simple assignment と simple call (`Call ...`, bare call, parenthesized call) を構造化し、raw line のインデント offset を sub-range に反映した
  - `packages/core/src/inference/inferModuleTypes.ts`、`packages/core/src/diagnostics/byRefDiagnostics.ts`、`packages/core/src/diagnostics/localVariableUsage.ts`、`packages/core/src/diagnostics/analyzeModule.ts`、`packages/core/src/diagnostics/duplicateDefinitions.ts`、`packages/core/src/symbol/buildModuleSymbols.ts`、`packages/server/src/lsp/documentService.ts` を更新し、新しい statement kind を既存の type inference / ByRef / reference / rename 系フローへ接続した
  - `packages/core/test/analysis.test.ts` に parser shape の直接検証を追加し、assignment / call structured AST が出ることを固定した
  - `npm run test --workspace @vba/core`、`npm run test --workspace @vba/server`、`npm run lint`、`npm run package`、`npm run test --workspace vba-extension` が成功した

- [x] worksheet control shapeName path の semantic case spec を最小抽出する
  - [test-support/worksheetControlShapeNamePathCaseTables.cjs](test-support/worksheetControlShapeNamePathCaseTables.cjs) に `semantic.positive` / `semantic.negative` を追加し、既存の `fixture` / `anchor` / `rootKind` / `routeKind` / `scopes` を completion / hover / signature と共有したまま `identifier` / `tokenKind` だけを semantic 用に持たせた
  - [scripts/test/worksheetControlShapeNamePathCaseTables.test.mjs](scripts/test/worksheetControlShapeNamePathCaseTables.test.mjs) を更新し、semantic でも両 route、`document-module/static/matched`、negative reason 一式、`property` / `method` token kind、両 fixture、extension / server scope を自動検証するようにした
  - [packages/server/test/documentService.test.js](packages/server/test/documentService.test.js) と [packages/extension/test/suite/index.ts](packages/extension/test/suite/index.ts) を更新し、worksheet control shapeName path の semantic token assertion を shared case spec から消費する helper を追加した
  - [packages/server/src/lsp/documentService.ts](packages/server/src/lsp/documentService.ts) では semantic token を同一 range 1 件へ集約し、function return の synthetic declaration token を除外して overlap を防ぐようにした
  - `npm run test:scripts`、`npm run test --workspace @vba/core`、`npm run test --workspace @vba/server` は成功し、`npm run test --workspace vba-extension` は hang / timeout 切り分け中のため継続タスクへ移した

- [x] 整理系 skill と subagent の責務分離を導入する
  - `.codex/skills/doc-minimum-update`、`.codex/skills/lightweight-review`、`.codex/skills/task-close-check` と `.codex/skills/README.md` を追加し、整理系作業を repo-local skill として扱う入口を作った
  - [AGENTS.md](AGENTS.md)、[docs/process/sub-agent-escalation.md](docs/process/sub-agent-escalation.md)、[docs/process/README.md](docs/process/README.md) を更新し、親エージェントが実装責任を持ち、subagent は `impact-investigator` / `skill-gatekeeper` / `diff-reviewer` の限定用途にだけ使う運用へ寄せた
  - [TASKS.md](TASKS.md) の運用ルールへ、整理単独で完了扱いにしないこと、文書更新は実装差分に付随する最小限に留めること、整理系 skill は条件付き実行に限ることを追記した
  - この作業はユーザー明示の運用更新依頼として実施し、product build / lint / test ではなく、skill frontmatter、必須見出し、相互参照の整合を確認した
