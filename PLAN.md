...
- 現在の主実装軸は Phase 3 の AST 安定化・構文情報整備を完了し、AST を後続フェーズへ接続する段階へ移る
- 直近完了は Phase 3 の type inference assignment fallback 削除、member call statement structured AST 化、formatter compressed block 判定の structured kind 接続、local rename target range の structured segment 優先化、active workbook identity snapshot transport の安定化
...
2. `CreateObject("WScript.Shell")` 既知 ProgID 解決の最小実装へ進む
...
1. `CreateObject("WScript.Shell")` 既知 ProgID 解決の最小実装
2. symbol / scope 抽出の structured statement 対応を維持する回帰追加
3. diagnostics の text 走査依存を、実装済み AST segment から外れた未構造化 statement に限定する
