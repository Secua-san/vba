...
- 現在の主実装軸は Phase 5 の名前解決・基本型推論を完了し、diagnostics と補完の AST / symbol / type 接続を強化する段階へ移る
- 直近完了は Phase 5 の `CreateObject("WScript.Shell")` 既知 ProgID 解決と built-in member surface の回帰固定
...
1. diagnostics の text 走査依存を、実装済み AST segment から外れた未構造化 statement に限定する
2. 補完の文脈絞り込みと誤候補抑制を強化する
