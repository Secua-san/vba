...
- 現在の主実装軸は Phase 8 の VBA 特有 runtime binding 戦略を完了し、Phase 9 の定義ジャンプ・参照検索・シンボルナビゲーション強化へ移る
...
2. Phase 9 の definition / references / rename / document symbol の安定度と coverage を上げる
...
1. definition / references / rename / document symbol の coverage 拡大
2. workbook / sheet / control 系の既存 sidecar 連携の coverage 拡大
3. `vbac.wsf` の combine 安全性と失敗時ログの強化

## 次の実装候補

次タスクは、原則として次の順で選ぶ。

1. Phase 9 の definition / references / rename / document symbol の安定度と coverage を上げる
2. core / server / extension の回帰を維持しながら structured AST と symbol resolution 利用箇所を保守する
