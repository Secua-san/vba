...
- 直近完了は `assignment` / `call` の structured AST first slice、主要 block statement structured node / block validation の first slice、active workbook identity snapshot transport の安定化
...
2. formatter / navigation / references / rename / semantic token に残る block 判定を AST kind ベースへ寄せる
3. structured AST 対応の parser / core / server / extension 回帰を追加する
...
1. structured statement を使う diagnostics / symbol 連携の次 slice
2. `formatModuleIndentation.ts` に残る block text 判定の段階移行
3. references / rename / semantic token の residual text 依存削減
4. structured AST 対応の parser / core / server / extension 回帰追加
5. `CreateObject("WScript.Shell")` 既知 ProgID 解決の最小実装
