...
- 現在の主実装軸は Phase 4 のシンボルテーブル・スコープ解析を完了し、名前解決・基本型推論と VBA / Excel 固有機能へ接続する段階へ移る
- 直近完了は Phase 4 の same-kind shadowing 対応、symbol resolution 共通化、definition / references / rename の回帰固定
...
1. diagnostics の text 走査依存を段階的に削減
2. type inference の statement / expression 粒度を上げる
3. 補完の文脈絞り込みと誤候補抑制を強化する
...
2. diagnostics の text 走査依存を、実装済み AST segment から外れた未構造化 statement に限定する
