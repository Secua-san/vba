...
- 現在の主実装軸は `ProcedureStatementNode` の block statement structured AST 拡張
- 直近完了は `assignment` / `call` の structured AST first slice と、active workbook identity snapshot transport の安定化
...
| Phase 2: 構文解析 MVP | 進行中 | module / procedure / declare / enum / type / variable に加え、block statement を node kind 化し始めている | `If` / `Select Case` / `For` / `For Each` / `Do` / `While` / `With` / `On Error` の structured node を揃える |
| Phase 3: AST 安定化・構文情報整備 | 進行中 | `range` / `headerRange` / `nameRange` はあるが、procedure body はまだ flat list 中心で block 関係を後段 stack 判定に頼る部分が残る | block validation と後続処理を AST kind ベースへ寄せる |
...
このリポジトリは user-facing 機能が先行しているため、当面の最優先は Phase 2-3 の負債返済である。  
特に `ProcedureStatementNode` の block statement structured AST を広げ、raw text 判定を減らす。

#### 直近に実装する順序

1. `If` / `Select Case` / `For` / `For Each` の structured node を揃える
2. `Do` / `While` / `With` / `On Error` を続けて node kind 化する
3. block validation を raw text regex から AST kind ベースへ移す
4. formatter / diagnostics / navigation / references / rename / semantic token の回帰を確認する
...
1. `ProcedureStatementNode` の `If` / `Select Case` / `For` / `For Each` structured node 追加
2. `ProcedureStatementNode` の `Do` / `While` / `With` / `On Error` structured node 追加
3. block validation の AST kind ベース移行
4. structured AST 対応の parser / core / server 回帰追加
5. structured statement を使う diagnostics / symbol 連携の first slice
6. `CreateObject("WScript.Shell")` 既知 ProgID 解決の最小実装
