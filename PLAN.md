...
- 現在の主実装軸は Phase 7 の IntelliSense / 補完 MVP を完了し、Phase 8 の VBA 特有 runtime binding 戦略へ移る
- 直近完了は completion のコメント / 文字列内抑制と、明示型ローカル変数からの built-in member 補完接続
...
1. `CreateObject` / ProgID 系など VBA 特有の runtime binding 戦略を追加する
2. `Variant` / `Object` の暫定補完戦略を検討する
