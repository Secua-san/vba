...
| 非実装作業の委譲 / 判断が空転した | [sub-agent-escalation.md](sub-agent-escalation.md) |
...
- `.frx` / host bridge 設計ゲート: [frx-and-real-host-bridge-design-gate.md](frx-and-real-host-bridge-design-gate.md)
- Parser / analysis performance 系: [incremental-analysis-feasibility.md](incremental-analysis-feasibility.md)
...
- 優先度確認、branch 状態確認、slice 比較、既存パターン探索、再現確認のような非実装作業は、まず [sub-agent-escalation.md](sub-agent-escalation.md) に従って適切な subagent へ委譲する。
