...
- broad root family の v1 対象構文は `Worksheets("literal sheetName")` と `Application.Worksheets("literal sheetName")` に限る。`Sheets`、`ActiveSheet`、numeric selector、dynamic selector、grouped selector は broad root sidecar lookup の対象にしない。
