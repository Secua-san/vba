...
- `Application.ThisWorkbook` は Office VBA の正本で current macro code が動いている workbook を返すため、explicit `Application.` qualifier を挟んでも `ThisWorkbook` と同じ workbook identity を維持できる。
...
- `Application.ThisWorkbook.Worksheets("Sheet1")` と `Application.ThisWorkbook.Worksheets.Item("Sheet1")` は、`ThisWorkbook` direct root と同じ static current-bundle family として扱い、manifest / snapshot 非依存で current bundle sidecar lookup の候補へ開いてよい。
...
- `Application.ActiveWorkbook.Worksheets("Sheet1")` と `Application.ActiveWorkbook.Worksheets.Item("Sheet1")` は、`ActiveWorkbook` direct root と完全に同じ active-workbook family として扱い、同じ 4 条件がそろったときだけ user-facing にしてよい。
...
- `Application.ThisWorkbook` / `Application.ActiveWorkbook` を含む sidecar lookup は、`Application` root が built-in `Application` として解決できた場合にだけ適用し、同名の変数、関数、メンバーへ shadow されているときは user-defined symbol を優先する。
...
- `Application.ThisWorkbook...` を `ThisWorkbook...` と同じ family に含めることで、workbook root の書き方が明示されているだけで static current-bundle 解決が閉じる不整合を避けやすい。
...
- `Application.ActiveWorkbook...` を `ActiveWorkbook...` と同じ family に含めることで、明示 qualifier の有無だけで broad-root gating 条件が変わる不整合を避けやすい。
...
- 調査詳細や除外境界の補足は `docs/process/shape-oleformat-object-explicit-sheet-root-feasibility.md`、`docs/process/explicit-sheet-name-broad-root-feasibility.md`、`docs/process/application-workbook-root-feasibility.md` を参照する。
